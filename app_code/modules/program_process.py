...
class ProgramProcess:
    def __init__(self, source_log_file, api_key, result_log_path, result_log_format):
        ...
        virus_analyzer = VirusTotalChecker(api_key, source_log_parser.matched_data)

        if result_log_format == 'csv':
            csv_report = CsvReportGenerator(virus_analyzer.checks_result, result_log_path)
            csv_report.generate()
        elif result_log_format == 'json':
            json_report = JsonReportGenerator(virus_analyzer.checks_result, result_log_path)
